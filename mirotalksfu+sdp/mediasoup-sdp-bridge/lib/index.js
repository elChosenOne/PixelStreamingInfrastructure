...
class SdpEndpoint {
    constructor(webRtcTransport, localCaps) {
        //console.error("Creando SdpEndpoint", webRtcTransport);
        this.producers = [];
        this.producerMedias = [];
        this.consumers = [];
    ...
    async processOffer(sdpOffer) {
        ...
        //console.log("WebRtcTransport.connect() with remote SDP offer", this.webRtcTransport);
        ...
        for (const media of remoteSdpObj.media) {
            ...
            if (media.type == "application") {
                ...
                console.log("[SdpEndpoint.processOffer] mediasoup Producer created, kind: %s, type: %s, paused: %s, id; %s", producer.kind, producer.type, producer.paused, producer.id);
            }
        }
