...
function forwardSFUMessageToPlayer2(msg) {
	const playerId = getPlayerIdFromMessage(msg);
	const player = players.get(playerId);
	if (player) {
		...
		if (data.length > 0) {
			...
			if (GRTPCapabilities && GTransportID) {
				...
				for (let { producer_id, peer_name, peer_info, type, producerTransportId, sdpOffer } of data) {
					...
					//console.log("Producer transport ID:", producerTransportId);
					console.log("SDP Offer: ", sdpOffer);
					//console.log("Player Component: ", playerComponent);

					playerComponent.sendFrom(sdpOffer);
					//forwardSFUMessageToPlayer2(sdpOffer)

					/*BTSocket.request('SDPConsume', {
